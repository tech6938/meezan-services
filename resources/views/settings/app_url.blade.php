    ...
    <div class="main-content">
        <section class="section">
            <div class="d-flex justify-content-end pb-3">
                <a href="" class="btn btn-primary text-white" data-toggle="modal" data-target="#addMainCategoryModal">+
                    Add New</a>
            </div>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <td>
                                                            <div class="d-flex gap-2">
                                                                <form action="{{ route('appUrl.destroy', $item->id) }}"
                                                                ...
                                                                <div class="p-1">
                                                                    <button type="button" class=" btn btn-info"
                                                                        data-toggle="modal"
                                                                        data-target="#editMainCategoryModal{{ $item->id }}">
                                                                        <i data-feather="edit-3"></i>
                                                                    </button>
                                                                </div>
                                                                {{-- <div class="p-1">
                                                                    <a href="{{ route('main-categories.show', ['main_category' => $item->id]) }}"
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Add App Url</h5>
                </div>
                <form action="{{ route('appUrl.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label>App Url</label>
                            <input type="url" class="form-control" name="app_url" required>
                        </div>
                        <div class="form-group">
                            <label>Privacy Policy</label>
                            <input type="text" class="form-control" name="privacy_policy" required>
                        </div>
    ...
    @foreach ($data as $item)
        <div class="modal fade" id="editMainCategoryModal{{ $item->id }}" tabindex="-1" aria-hidden="true">
            <div class="modal-dialog modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title">Edit App Url</h5>
                    </div>
                    <form action="{{ route('appUrl.store') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('POST')
                        <div class="modal-body">
                            ...
                            <div class="form-group">
                                <label>App Url</label>
                                <input type="url" class="form-control" name="app_url" value="{{ $item->app_url }}"
                                    required>
                            </div>
                            <div class="form-group">
                                <label>Privacy Policy</label>
                                <input type="text" class="form-control" name="privacy_policy"
                                    value="{{ $item->privacy_policy }}" required>
                            </div>
