...
@section('content')
                                            ...
                                            @if ($data->isNotEmpty())
                                                @foreach ($data as $item)
                                                    <tr>
                                                        ...
                                                        <td>{{ $item->name }}</td>
                                                        <td>
                                                            <img src="{{ $item->profile_image ?? asset('profiles/default.jpg') }}"
                                                                style="width:80px; height:80px; object-fit:cover;"
                                                                class="rounded" alt="profile image">
                                                        </td>
                                                        <td>
                                                            <div class="badge badge-info">{{ $item->phone }}</div>
                                                        </td>
                                                        <td>{{ $item->email }}</td>
                                                        <td class="d-flex align-items-center gap-2">
                                                            {{-- Action buttons --}}
                                                            <button class="btn btn-primary openModalBtn"
                                                                data-provider-id="{{ $item->id }}"><i
                                                                    data-feather="edit-2"></i></button>
                                                            <!-- In your table row, pass files as JSON -->
                                                            <a href="{{ route('shops', ['id' => $item->id]) }}"
                                                                class="btn btn-dark viewBtn">
                                                                <i data-feather="eye"></i>
                                                            </a>
                                                            {{-- <form
                                                                action="{{ route('shopkeepers.destroy',['id' => $item->id]) }}"
    ...
    <div id="myModal" class="modal">
        <div class="modal-content">
            ...
            <h3>Select Status</h3>
            <form action="{{ route('shopkeepers.status') }}" method="post">
                @csrf
                <input type="hidden" id="shopkeeperIdInput" name="shopKeeper_id" value="">
...
@section('js')
    <script>
        ...
        const shopkeeperIdInput = document.getElementById("shopkeeperIdInput");
        ...
        document.querySelectorAll(".openModalBtn").forEach(btn => {
            btn.addEventListener("click", function() {
                shopkeeperIdInput.value = this.dataset.providerId;
